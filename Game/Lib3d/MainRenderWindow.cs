...
namespace Lib3d
{
    ...
    public class MainRenderWindow(int width, int height, string title, double fps)// : GameWindow
    {
        ...
        /// </summary>
        public void Render3DObjects(Minecraft_Clone_Tutorial_Series_videoproj.Camera camera)
        {

            
            if (UseDepthTest) GL.Enable(EnableCap.DepthTest);
            foreach (var obj in _mainObjects) obj.Show(camera);
            foreach (var obj in _mainTexturedObjects) obj.Show(camera);
        ...
        /// </summary>
        public void RenderLight(Minecraft_Clone_Tutorial_Series_videoproj.Camera camera)
        {
            _mainLamp.Show(camera, _lampShader);
        }
        ...
        public int CreateCube(Color4 color, float width, float height, float depth)
        {
            ...
            //color = new Color4(255,0,0,255);
            var cubeVertex = CreateRectangularPrismVerticesLines(width, height, depth);
        ...
        private static float[] CreateRectangularPrismVerticesLines(float width, float height, float depth)
        {
            var w = width / 2;
            var h = height / 2;
            var d = depth / 2;
        ...
        public void DrawLine(float x1, float y1, float x2, float y2, Color4 color)
        {
            var x1Trans = x1 - Size.X / 2;
            var y1Trans = y1 - Size.Y / 2;
            var x1Norm = x1Trans / (Size.X / 2);
        ...
        private class Object
        {
            ...
            public void Show(Minecraft_Clone_Tutorial_Series_videoproj.Camera camera)
            {
                GL.BindVertexArray(_mainObject);
                ...
                //_shader.SetVector3("lightColor", new Vector3(1,1,1));
                //_shader.SetVector3("lightPos", new Vector3(0,0,0));
        ...
        private class Lamp
        {
            ...
            public void Show(Minecraft_Clone_Tutorial_Series_videoproj.Camera camera, Shader lampShader)
            {
                GL.BindVertexArray(_mainObject);
